    ...
    <div class="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-purple-100">
        <div class="flex justify-center mb-6">
            <div class="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center">
                <img src="{{ asset('images/usuario.png')}}" alt="usuario.jpg">
            </div>
        </div>

        <h2 class="text-2xl font-semibold text-center text-gray-700 mb-8">Crear Nueva Cuenta</h2>

        <form action="{{ route('user.store') }}" method="POST" class="space-y-5">
            @csrf

            <div class="border-black">
                <label for="name" class="block text-sm font-medium text-gray-600 mb-1 ml-1">Nombre completo</label>
                <input type="text" 
                       name="name" 
                       id="name" 
                       value="{{ old('name') }}" 
            ...
            <div class="">
                <label for="email" class="block text-sm font-medium text-gray-600 mb-1 ml-1">Correo</label>
                <input type="email" 
                       name="email" 
                       id="email" 
                       value="{{ old('email') }}" 
            ...
            <div class="border-black">
                <label for="password" class="block text-sm font-medium text-gray-600 mb-1 ml-1">Contraseña</label>
                <input type="password" 
                       name="password" 
                       id="password" 
                       required 
